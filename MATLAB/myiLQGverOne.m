function [x, u, L, l, Vx, Vxx, cost, trace, stop,Quu,QuuF] = myiLQGverOne(DYNCST, x0, u0, Op)
...
defaults = {'lims',           [],...            control limits
    ...
    'tolFun',         1e-6,...          reduction exit criterion
...
% initialize trace data structure, save All information at every
% iteration
...
for iter = 1:Op.maxIter
    ...
    % step 1. differentiate dynamcis and cost along new trajectory
    if flgChange
        t_diff = tic;
    ...
    % step2. backward pass, compute optimal control law and cost-to-go
    backPassDone = 0;
    while ~backPassDone
        t_back = tic;
        [diverge, Vx, Vxx, l, L, dV,Quu,QuuF] = back_pass(cx,cu,cxx,cxu,cuu,fx,fu,fxx,fxu,fuu,lambda,Op.regType,Op.lims,u);
...
function [diverge, Vx, Vxx, k, K, dV,Quu_save,QuuF_save] = back_pass(cx,cu,cxx,cxu,cuu,fx,fu,fxx,fxu,fuu,lambda,regType,lims,u)
...
Quu_save = zeros(m,m,N-1);
QuuF_save = zeros(m,m,N-1);

% The value of the final time
Vx(:,N) = cx(:,N);
Vxx(:,:,N) = cxx(:,:,N);
