...
class iLQR:
    ...
    def forward(self,x0,u,K,x,k,alpha):
        ...
        for i in range(N):
            ...
            cnew[i] = self.cost.estimateCost(xnew[i,:],unew[i,:])
        
        cnew[N] = self.cost.estimateCost(xnew[N,:],np.zeros(self.model.iu))
    ...
    def update(self,x0,u0):
        ...
        for j in range(np.size(self.Alpha,axis=0)):
            for i in range(self.N):
                ...
                self.c[i] = self.cost.estimateCost(self.x[i,:],self.Alpha[j]*self.u[i,:])
        ...
        for iteration in range(self.maxIter) :
            ...
            if flgChange == True:
                ...
                c_x_u = self.cost.diffCost(self.x[0:N,:],self.u)
                c_xx_uu = self.cost.hessCost(self.x[0:N,:],self.u)
                ...
                c_x_u = self.cost.diffCost(self.x[N,:],np.zeros(iu))
                c_xx_uu = self.cost.hessCost(self.x[N,:],np.zeros(iu))
